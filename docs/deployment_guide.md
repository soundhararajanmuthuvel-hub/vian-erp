# VIAN ERP - Production Deployment Guide (Vercel + Render + Render PostgreSQL)
...
graph TD
    ...
    Render -->|Sequelize TLS/SSL Pool| Postgres[(Render Managed PostgreSQL)]
...
3. **Database**: [Render Managed PostgreSQL](https://render.com/) managed relational database with enforced TLS/SSL encryption.
...
- A **Render PostgreSQL** managed database instance (`vian-erp-db`).
...
- 🚀 **[Render PostgreSQL & Backend Deploy Guide](render_guide.md)**: Detailed step-by-step setup for Render Web Service and Render PostgreSQL.
