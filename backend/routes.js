...
function registerRoutes(app, models) {
  ...
  app.post('/api/generate-link', authenticateToken, async (req, res) => {
    ...
    try {
      ...
      if (link) {
        await link.update({ token, expiryDate, status: 'Active', qrCodeData: publicUrl });
      } else {
        link = await PublicEnquiryLink.create({ leadId, token, expiryDate, status: 'Active', qrCodeData: publicUrl });
      }
      ...
      res.json({ success: true, token, publicUrl, link });
    } catch (err) {
  ...
  app.post('/api/enquiry/generate-link', authenticateToken, async (req, res) => {
    const { leadId, expiryDays } = req.body;
    if (!leadId) return res.status(400).json({ message: 'Lead ID is required' });
    try {
      const lead = await Lead.findByPk(leadId);
  ...
  // Public Secure Link detail fetch endpoint
  app.get('/api/public-enquiry/:token', async (req, res) => {
    try {
      ...
      if (link.status === 'Inactive') {
        return res.status(400).json({ message: 'This secure link has been deactivated' });
      }
      if (link.expiryDate && new Date(link.expiryDate) < new Date()) {
        return res.status(400).json({ message: 'This secure link has expired' });
      }
      ...
      res.status(500).json({ message: 'Error retrieving public secure link details', error: err.message });
    }
  });

  // Alternative Public Enquiry Submit
  app.post('/api/public-enquiry/:token/submit', async (req, res) => {
    try {
      const { token } = req.params;
      const link = await PublicEnquiryLink.findOne({ where: { token } });
      if (!link) return res.status(404).json({ message: 'Invalid secure link' });
      if (link.status === 'Inactive') return res.status(400).json({ message: 'Secure link is inactive' });
      if (link.expiryDate && new Date(link.expiryDate) < new Date()) {
        return res.status(400).json({ message: 'Secure link has expired' });
      }
      ...
      await PublicEnquiryHistory.create({
        ...
        notes: `Enquiry form submitted by client from IP ${clientIp}`,
        clientIp
      });
